...
struct block *findFreeBlock(struct block **last, size_t size) 
{
   ...
   /*
    while you havent gone through all of memory
    sort through if the block is free calculate diff
    if lesser than the current least difference 
   ...
   */
   ...
   while (curr ) 
   {
      ...
      if(curr->free){
      if(diff < ( curr->size - size) && (curr->size - size)>0){
        chosen=curr;
        diff= (curr->size - size );
   ...
   curr=chosen;
    ...
    while you havent gone through all of memory
    sort through if the block is free calculate diff
    if greater than the current least difference 
    update the chosen one
   */
...
while (curr ) 
   ...
   printf("TODO: Implement worst fit here\n");
    ...
    /*
     I know  i changed something I just forgot what

     The difference from  first and next is where they begin and
   */
    while (curr && !(curr->free && curr->size >= size)) 
   {
      *last = curr;
...
void *malloc(size_t size) 
{
   ...
   /* if there is room left than allocate a new block
      assign them to the excess space
      change the  end pointer of new one to the next one
      current block next is the new block. update the number of blocks
   */

   if(next->size > size){
      struct block *excess=(struct block *)sbrk(sizeof(struct block) + (next->size - size));
      
      next->size= size;
      excess->size= (next->size - size);
      excess->next= next->next;
      next->next= excess;
...
void free(void *ptr) 
{
   ...
   /* if next block is free too, consolidate value and next
   delete second struct after you make the next pointer to next one


  ***** I think I had to check for something else but I forgot
   */
