...
void View::checked_out_items(vector<Customer> customers)
{
	vector<Media> m;
	for(int i = 0; i < customers.size(); i++)
	{
		cout << customers[i].get_name() << " checked out items: " << endl;
		cout << "------------------------------" << endl;
		if(customers[i].get_checked_out_medias().size() == 0 && customers[i].get_checked_out_bundles().size() == 0)
			cout << customers[i].get_name() << " does not have any items checked out." << endl << endl;
		for(int count = 0; count < customers[i].get_checked_out_medias().size(); count++)
				cout << customers[i].get_checked_out_medias()[count].get_title() << endl;
		for(int counter = 0; counter < customers[i].get_checked_out_bundles().size(); counter++)
		{
			m = customers[i].get_checked_out_bundles()[counter].get_medias();
			for(int c = 0; c < m.size(); c++)
				cout << m[c].get_title() << endl;
			m.clear();
		}
	}
	cout << endl;
