...
bool Extendable_Arm::move(int x, int y)
{
	...
	if (distance2 > retract_length)
	{
		...
		{
			cout << "Position is out of extended arm range of " << length << " units." << endl << endl;
			return false;
		}
	}
	...
	if (power > battery_level)
	{
		...
		position.first = x2;
		position.second = y2;
		cout << "Extendable Arm Robot 1003 was able to get to coordinates (" << x2 << "," << y2 << ") before running out of power." << endl << endl;
		return false;
	}
	// Assign values
	position.first = x;
